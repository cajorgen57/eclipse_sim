...
class HexTile:
    ...
    Represents a tile before placement. After placement, it becomes a Hex
    in the MapState with applied rotation and coordinates.
    """

    id: str
    ring: int
    wormholes: Sequence[int]  # Edge indices (0-5) that have wormholes in base orientation
    symbols: Sequence[str] = field(default_factory=tuple)
    warp_portal: bool = False
    gcds: bool = False
    vp: int = 0  # Victory points printed on tile
    discovery_slots: int = 0  # Number of discovery tile slots
    ancients: int = 0  # Number of Ancient ship icons
    artifacts: int = 0  # Number of artifact icons (for Artifact Key tech)
    tile_number: int = 0  # Combat ordering number (higher = earlier, center = last)
    pop: Dict[str, int] = field(default_factory=dict)  # Population squares by type
