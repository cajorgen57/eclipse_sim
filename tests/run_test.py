"""
Orion Turn 1 Test - Uses improved evaluation framework with 80+ features

This test demonstrates the AI's decision-making on Orion's opening position.
Now uses the comprehensive evaluation system with strategy profiles.

Usage:
    # Basic test with improved defaults (600 sims, depth 3)
    python tests/run_test.py
    
    # With verbose output showing features and recommendations
    ...
    python tests/run_test.py --output results.json
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Orion Turn 1 Test - Uses improved evaluation with 80+ features"
    )
    ...
    parser.add_argument("--sims", type=int, default=600, help="Simulations (improved default: 600)")
    parser.add_argument("--depth", type=int, default=3, help="Search depth (improved default: 3)")
    parser.add_argument("--topk", type=int, default=5)
    parser.add_argument("--profile", help="Strategy profile: aggressive, economic, tech_rush, defensive, balanced")
    parser.add_argument("--verbose", "-v", action="store_true", help="Show detailed analysis with features")
    parser.add_argument(
        "--output",
    ...
    # Build manual inputs with improved defaults and profile support
    manual_inputs = {
        "_planner": {
    ...
    if args.profile:
        manual_inputs["_profile"] = args.profile
        if args.verbose:
            print(f"\n🎯 Using strategy profile: {args.profile}")
    
    # Show verbose info if requested
    if args.verbose:
        print("\n" + "="*70)
        print("ORION TURN 1 TEST - EVALUATION FRAMEWORK")
        print("="*70)
        print(f"\nConfiguration:")
        ...
        try:
            ...
            feats = features.extract_features(base_state)
            print(f"\n📊 Feature Extraction: {len(feats)} features")
            
            # Show key features
            key_features = ["money", "science", "materials", "total_fleet_size", 
                          "controlled_hexes", "tech_count", "fleet_power",
                          "money_income", "science_income"]
            print("  Key game state features:")
            for feat_name in key_features:
                if feat_name in feats:
                    print(f"    {feat_name:20s} = {feats[feat_name]:.2f}")
        except Exception as e:
            print(f"  (Feature extraction info unavailable: {e})")
    ...
    # Show results: verbose mode for humans, JSON for machines
    if args.verbose:
        print("="*70)
        print("🎯 RECOMMENDATIONS")
        print("="*70)
        
        for i, plan in enumerate(summary.get("plans", []), 1):
            print(f"\nPlan {i}:")
            ...
            for j, step in enumerate(steps[:3], 1):  # Show first 3 steps
                ...
                print(f"  {j}. {action}", end="")
                
                # Show action-specific details
                if action == "RESEARCH" and "tech" in payload:
                    print(f" → {payload['tech']}")
                    if "category" in details:
                        print(f"     Category: {details['category']}, Multiplier: {details.get('multiplier', 1.0):.2f}")
                elif action == "BUILD" and "ships" in payload:
                    ships_str = ", ".join(f"{n}× {cls}" for cls, n in payload["ships"].items() if n > 0)
                    print(f" → {ships_str}")
                elif action == "EXPLORE":
                    ring = payload.get("ring", "?")
                    print(f" → Ring {ring}")
                    if "ev_multiplier" in details:
                        print(f"     EV Multiplier: {details['ev_multiplier']}, Risk: {step.get('risk', 0):.2f}")
                elif action == "MOVE":
                    dst = payload.get("to", "?")
                    print(f" → to {dst}")
                else:
        ...
        print(f"  • {len(feats if 'feats' in locals() else {})} extracted features")
        print(f"  • {args.sims} simulations at depth {args.depth}")
        print(f"  • Profile: {args.profile or 'balanced'}")
        print(f"  • Generated {len(summary.get('plans', []))} plan options\n")
    else:
