...
GameState {
  ...
  map: MapState  # hexes with axial coordinates, wormholes, ownership
...
Hex {
  id: str
  ring: int  # 0=center, 1=inner, 2=middle, 3+=outer
  axial_q: int  # Axial q coordinate (horizontal E-W)
  axial_r: int  # Axial r coordinate (diagonal NE-SW)
  wormholes: [int]  # Edge indices 0-5 with wormholes (after rotation)
  rotation: int  # Rotation applied when placed (0-5)
  neighbors: {edge -> hex_id}  # Adjacent hexes
  pieces: {player_id -> Pieces}  # Ships, discs, colonies
  ancients: int  # Ancient ships present
  discovery_tile: str  # Discovery state ("pending", "placed:<id>", etc.)
}
```
...
* `Explore`: allowed only across a wormhole edge; draw from the correct sector stack【turn1file2†】【turn1file2†】.
  - Implementation: Uses axial coordinates to find adjacent unexplored positions
  - Determines sector (I/II/III) via `ring_radius()` function
  - Validates placement by trying all 6 rotations to find wormhole match
  - See `eclipse_ai/action_gen/explore.py` and `eclipse_ai/map/placement.py`
...
plans = recommend(
  ...
  prior_state=None,
  manual_inputs={"_planner":{"simulations":600,"depth":3}},
  top_k=5,
  planner="pw_mcts"  # Only supported planner
)
```
...
3. `PW_MCTSPlanner.plan(state)` → ranked actions
